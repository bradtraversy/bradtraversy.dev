...
dek: what travxlabs.com is, what it isn't, and why i'm building it in the open
publishDate: 2026-05-06
draft: true
tags: [meta, agents]
relatedProjects: [travxlabs-site]
---

travxlabs.com is the public home of travx labs — the umbrella for the side projects, dev tools, and saas products i build outside of traversy media.

## what's here

four kinds of content, each pulling its weight differently:

- **articles** — polished writeups. low cadence, high quality. "how travis (my agent) actually works", "from prototype to prod in 6 weeks", that kind of thing.
...
a paid saas with real revenue. half a dozen free dev tools. an open-source cli. an internal dashboard that runs my homelab. an agent setup with claude and openclaw that's deeply non-standard. the occasional weekend prototype that gets killed two days later.
...
travxlabs.com is that surface. one page per project, with the build documented in the open. one stream of devlog entries pulled from real session notes. a catalog of tools so the small things don't just become a tweet that scrolled away.
...
i use claude code (in the terminal, on this laptop), cowork (claude desktop, on my mac), and travis (an autonomous agent running on my homelab via openclaw). they share a vault. they read each other's session logs. they pick up tasks from each other through a shared task queue.

it's not a research project. it's the actual setup that ships vidpipe, polishes devlog drafts, writes commit messages, and keeps the whole operation moving. distinctive enough that it earns its own article — "how travis works" — coming soon.

## what travxlabs isn't
...
- **not a marketing site.** vidpipe is paid, but you won't find pricing here — pricing lives on vidpipe's own page. travxlabs is the lab; the actual product is somewhere else.
