...
`Referer: localhost:3000/` header. that's youtube's anti-hotlinking.
without the referer, 200 every time. fix is one attribute on the
`<img>`: `referrerPolicy="no-referrer"`. worth knowing for any
embedded google cdn asset.
